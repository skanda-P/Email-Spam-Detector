...
# ─────────────────────────────────────────────
# 1. Feature extraction (mirrors training code)
# ─────────────────────────────────────────────
...
_PRICE_RE    = re.compile(r'\$[\d,]+|\d+%\s*off')
...
def extract_features(text: str) -> list:
    ...
    html_tags = _HTML_TAG.findall(text)
    feats = {
        ...
        'has_price_pattern':    int(bool(_PRICE_RE.search(text))),
...
# ─────────────────────────────────────────────
# 2. Model definition (mirrors training code)
# ─────────────────────────────────────────────
NUM_FEATURES = 15
...
print(f"[startup] Loading tokenizer …")
tokenizer = AutoTokenizer.from_pretrained("roberta-base")

print(f"[startup] Loading encoder …")
encoder = AutoModel.from_pretrained("roberta-base")

print(f"[startup] Building model …")
model = HybridSpamClassifier(encoder=encoder).to(device)

print(f"[startup] Loading weights from {MODEL_PATH} …")
